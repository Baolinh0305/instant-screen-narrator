            ...
            egui::Frame::group(ui.style()).show(ui, |ui| {
                ...
                if self.selected_api == "gemini" {
                    ui.horizontal(|ui| {
                        ...
                        ui.text_edit_singleline(&mut self.gemini_api_key);
                    });
                } else {
                    ui.horizontal(|ui| {
                        ui.label("Nhập Groq api key:");
                        ui.text_edit_singleline(&mut self.groq_api_key);
                    });
                }
            });
            egui::Frame::group(ui.style()).show(ui, |ui| {
                ...
                egui::TextEdit::multiline(&mut self.current_prompt).desired_rows(5).show(ui);
            });
            ui.separator();
            egui::Frame::group(ui.style()).show(ui, |ui| {
                ...
                ui.horizontal(|ui| {
                    ui.label("Nút dịch:");
                    ui.text_edit_singleline(&mut self.hotkey_translate);
                });
                ui.horizontal(|ui| {
                    ui.label("Nút chọn vùng cần dịch:");
                    ui.text_edit_singleline(&mut self.hotkey_select);
                });
            });
            egui::Frame::group(ui.style()).show(ui, |ui| {
                ui.horizontal(|ui| {
                    ui.checkbox(&mut self.config.split_tts, "Split TTS");
...
fn main() -> Result<(), eframe::Error> {
    ...
    options.viewport.transparent = Some(true);
    options.viewport.inner_size = Some(egui::vec2(800.0, 700.0));
    options.viewport.taskbar = Some(false);
    eframe::run_native(
        "Screen Translator",
        options,
